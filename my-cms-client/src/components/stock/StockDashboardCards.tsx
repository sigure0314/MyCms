...
const MiniInstitutionalChart = ({data}:{data:StockDashboardViewModel['institutionalTrading']}) => { const values=data.foreign; const max=Math.max(...values.map(Math.abs)); return <div className="institutional-chart"><div className="chart-legend"><span className="foreign">外資</span><span className="trust">投信</span><span className="dealer">自營商</span></div><div className="bar-area">{values.map((value,i)=><div className="bar-column" key={data.labels[i]} title={`${data.labels[i]} 外資 ${formatInteger(value)} 張`}><i className={value>=0?'positive-bar':'negative-bar'} style={{height:`${Math.max(4,Math.abs(value)/max*46)}%`}} /></div>)}</div><Text type="secondary">單位：張；滑鼠移至柱狀圖查看資料</Text></div> };
